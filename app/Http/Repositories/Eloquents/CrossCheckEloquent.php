...
class CrossCheckEloquent extends BaseEloquent implements CrossCheckRepositoryInterface
{
    /**
     * get model
     * @return string
                    ...
                    foreach ($conditions['sort'] as $key => $value) {
                        if ($value['field'] == "order_id") {
                            if (strtolower($value["dir"] == 'asc')) {
                                $data = $data->orderBy("temp_order", 'desc');
                                $data = $data->orderBy("order_id", 'asc');
                                $data = $data->orderBy("active", 'desc');
                            } else {
                                $data = $data->orderBy("temp_order", 'asc');
