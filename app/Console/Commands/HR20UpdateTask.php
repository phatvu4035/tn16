...
class HR20UpdateTask extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:hr20-update-api';
    ...
    public function getFullData() {
        ...
        do {
            ...
            $skip = 500 * $index;
            $take = 500;
            $params = '?type=employee&skip='.$skip.'&take='.$take.'&is_include_statement=true&order_by=time_modified&is_get_latest_statement=true'.'&cutoff_date='.$currentDate;
            $url = $url . $params;       
    ...
    public function handleData($data) {
        foreach($data as $em) {
            ...
            if(isset($em['statements']) && $em['statements'] && count($em['statements']) ) {
                ...
                if(isset($em['statements'][0]['pt']) && $em['statements'][0]['pt']) {
                    $updatedData['phap_nhan'] = strtoupper(trim( explode( '-', $em['statements'][0]['pt'] )[0] ));
                }
          }
          ...
          if(!$updatedData['phap_nhan']) {
              $updatedData['phap_nhan'] = (isset($em['employee_corporation']) && $em['employee_corporation'] ) ? strtoupper($em['employee_corporation']) : '';
              if(!$updatedData['phap_nhan']) {
                  unset($updatedData['phap_nhan']);
              }
          }
